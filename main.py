...
def conversion_calculator(convert_from, convert_to, amount):
    try:
        conversion_value = data_conversion_dict[convert_from][convert_to]
...
def main():
    st.set_page_config(page_title="Weight Convertor")
    ...
    if submit:
        result = conversion_calculator(st.session_state.convert_from, st.session_state.convert_to, st.session_state.amount)
